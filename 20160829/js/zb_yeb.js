    ...
    function queryCode(code1,code2,str_codename,str_tableformat,bool_lastcode){
        ...
        $.ajax({
            ...
            data: {
                ...
                zth: p_zth,
                year: p_year,
                sql: 'select cclass,ccode,ccode_name,bend from code where (ccode between '+code1+' and '+code2+' or ccode like \''+code2+'%\') and ccode_name like \'%'+str_codename+'%\''+ str_sql_lastcode +' order by ccode'
            },
            success: function(d){
                console.log(d);
                if(d.status===1){
